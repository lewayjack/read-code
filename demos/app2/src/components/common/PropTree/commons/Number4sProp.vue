...
const number4sVueRef = ref<Number4s>([])
...
const inputHandler = (e: any, index: number, num: number) => {
    ...
    timers.value = setTimeout(() => {
        const number4Value = [...(JSON.parse(JSON.stringify(number4sVueRef.value)))] as Number4s
        number4Value[index][num] = Number(e.target.value)
        number4sVueRef.value = number4Value
    }, 200);
}

const addNumber = () => {
    number4sVueRef.value.push([0, 0, 0, 0])
}

const toNumber2s = () => {
    if (props.modelValue === undefined) {
        const arr = JSON.parse(JSON.stringify(props.defaultValue ?? [])) as Number4s
        number4sVueRef.value = [...arr]
    } else {
        const arr = JSON.parse(JSON.stringify(props.modelValue)) as Number4s
        number4sVueRef.value = [...arr]
    }
...
const ok = () => {
    const arr = JSON.parse(JSON.stringify(number4sVueRef.value)) as Number4s
    emits('update:modelValue', [...arr])
    viewsShow.value = false
}
const cancel = () => {
    number4sVueRef.value = []
    viewsShow.value = false
}
...
<template>
    <div class="prop_box">
        <div class="prop_edit">
            ...
            <span class="set_btn" @click.stop="toNumber2s" :title="`${readonly ? '查看' : '设置'}${propertyName}`">⚙</span>
        </div>
    ...
    <Window :show="viewsShow" :title="propertyName" :onlyButton="false" :readonly="readonly" :width="600" :height="400"
        ...
        <div class="prop_list" :class="{ 'readonly_style': readonly }">
            <div class="value_box">
                <span class="set_btn" @click.stop="addNumber()" title="添加" :class="{ 'readonly_true': readonly }">+</span>
            </div>
            <div class="value_box" v-for="(item, index) in number4sVueRef" :key="index">
                ...
                <input type="number" v-model.lazy="item[3]" @input="inputHandler($event, index, 3)" :disabled="readonly" />
                <span class="set_btn" @click.stop="number4sVueRef.splice(index, 1)" title="删除"
