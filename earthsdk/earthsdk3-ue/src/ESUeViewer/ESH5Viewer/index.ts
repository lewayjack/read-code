...
import { UTF8ToString } from "./utils";
let lastSendId = -1;
...
export class ESH5Viewer extends UeCloudViewerBase {
    ...
    private _dlViewer: DownloadViewer;
    get dlViewer() { return this._dlViewer; }
    constructor(container: HTMLDivElement, project: string, baseUrl: string = _baseUrl, private _ueViewer: ESUeViewer) {
        ...
        this._dlViewer = this.dv(new DownloadViewer(container, project, baseUrl, ueViewer));

        let ready = false;

        this.d(this._dlViewer.onRuntimeInitialized.don(() => {
            ready = true;
            // @ts-ignore
            if (window.UE_JSHTML5Communication) {
                //@ts-ignore
                window.UE_JSHTML5Communication.UESendMessageToJS = (str: any) => {
                    try {
                        const detailJson = JSON.parse(UTF8ToString(str));
                        if (ueViewer.debug) {
                            console.group(
                                "%cUE => JS",
        ...
        const processMessage = () => {
            if (!ready) return;
            //@ts-ignore
            if (!window.UE_JSHTML5Communication || !window.UE_JSHTML5Communication.JSSendMessageToUE) return;
            ...
            if (l < maxBytes) {
                ...
                //@ts-ignore
                window.UE_JSHTML5Communication.JSSendMessageToUE(infoStr);
                return
            }
            // 否则就分拆后再发送
