...
import { Credit, Event, WebMercatorTilingScheme, DefaultProxy, GeographicTilingScheme, Math as CMath, Resource, ImageryLayerFeatureInfo, defined, Rectangle, } from "cesium";
import { Coords, LayerSpecification, MVTImageryProviderOptions, StyleSpecification } from "./type";
//@ts-ignore
...
class MVTImageryProvider {
  ...
  constructor(options: MVTImageryProviderOptions & {
    ...
    this.rectangle = Rectangle.intersection(
      this.rectangle,
  ...
  private async _build(url: Resource | string | StyleSpecification, options: MVTImageryProviderOptions = {}) {
    ...
    style.layers = this.layerStyle.length != 0 ? <Array<LayerSpecification>>this.layerStyle : style.layers;
  ...
  private _preLoad(data: string | Resource | StyleSpecification): Promise<StyleSpecification> {
    ...
    if (typeof data === 'string') {
      if (data.endsWith(".json") || data.startsWith("mapbox://")) {
        data = new Resource({
          url: data
        })
      } else if (data.endsWith(".pbf")) {
        data = {
          "version": 8,
          "sources": {
            "customSource": {
          ...
          "layers": <Array<LayerSpecification>>this.layerStyle
        }
        promise = data
      }
    }
