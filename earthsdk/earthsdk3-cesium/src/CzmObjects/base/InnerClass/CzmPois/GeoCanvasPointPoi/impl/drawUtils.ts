 ...
 * @param fillStyle
 */
export function drawBubble(ctx: CanvasRenderingContext2D, mbx: number, mby: number, w: number, fillStyle: string) {
    ctx.beginPath();
    ctx.moveTo(mbx, mby);
 ...
 * 
 * @param ctx Context
 * @param centerX 底部中点x值
 * @param centerY 底部中点y值
 * @param radius 底部中点到圆心中点的距离
 * @param fillStyle
 */
export function drawPoint(ctx: CanvasRenderingContext2D, centerX: number, centerY: number, radius: number, fillStyle: string) {
    ctx.beginPath();
